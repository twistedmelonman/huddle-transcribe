...
**A PR that touches `.github/workflows/` gets no Codex review.** The skip
fires on *any* file matching `.github/workflows/*.yml` or `*.yaml` — not
only the Codex workflow itself. `Codex-action` refuses to run against
a PR that can modify the reviewer, so `Codex-blocking-review` reports a
...
only `lint.yml` skips too. An 8-second green `Codex-review / run-review` is
...
`Codex-blocking-review.yml` for the incident behind that choice; do not
